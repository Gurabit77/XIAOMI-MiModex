  ...
  return /* @__PURE__ */ jsx_dev_runtime.jsxDEV(jsx_dev_runtime.Fragment, {
    children: [
      /* @__PURE__ */ jsx_dev_runtime.jsxDEV(Dialog, {
        title: "Import MCP Servers from MiMo Desktop",
        subtitle: `Found ${serverNames.length} MCP ${plural(serverNames.length, "server")} in MiMo Desktop.`,
        color: "success",
...
async function mcpServeHandler({
  ...
  try {
    ...
    const { startMCPServer } = await import("./chunk-mcb3g4wd.js");
...
async function mcpListHandler() {
  ...
  if (Object.keys(configs).length === 0) {
    console.log("No MCP servers configured. Use `mimo mcp add` to add a server.");
  } else {
...
async function mcpAddFromDesktopHandler(options) {
  try {
    ...
    const { readClaudeDesktopMcpServers } = await import("./chunk-cx4ygprq.js");
    const servers = await readClaudeDesktopMcpServers();
    if (Object.keys(servers).length === 0) {
      cliOk("No MCP servers found in MiMo Desktop configuration or configuration file does not exist.");
    }
...
You will be prompted for approval next time you start MiMo Code.`);
}
export {
  mcpServeHandler,
